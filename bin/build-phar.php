...
namespace Program
{
class buildPharProgramInfo extends \Parser\ProgramInfo
{
	public function __construct()
	{
		parent::__construct("build-phar");
		
		$this->abstract = 'Create PHAR';
		...
		$G_1_g_3_skip_validation = new \Parser\SwitchOptionInfo("skipValidation", array('skip-validation', 'no-validation', 'S'), (0));
		
		$G_1_g_3_skip_validation->abstract = 'Skip XML Schema validations';
		
		$G_1_g_3_skip_validation->details = 'The default behavior of the program is to validate the given xml-based file(s) against its/their xml schema (http://xsd.nore.fr/program etc.). This option will disable schema validations';
		$G_1_g->appendOption($G_1_g_3_skip_validation);
		...
		$G_2_g_1_parser_namespace = new \Parser\ArgumentOptionInfo("parserNamespace", array('parser-namespace', 'parser-ns'), (0));
		
		$G_2_g_1_parser_namespace->abstract = 'PHP parser namespace';
		
		$G_2_g_1_parser_namespace->details = 'Namespace of all elements of the ns-xml PHP parser';
		$G_2_g->appendOption($G_2_g_1_parser_namespace);
		
		// prg:argument programNamespace
		$G_2_g_2_program_namespace = new \Parser\ArgumentOptionInfo("programNamespace", array('program-namespace', 'program-ns', 'prg-ns'), (0));
		
		$G_2_g_2_program_namespace->abstract = 'PHP program namespace';
		$G_2_g->appendOption($G_2_g_2_program_namespace);
		...
		$G_5_help = new \Parser\SwitchOptionInfo("displayHelp", array('help'), (0));
		
		$G_5_help->abstract = 'Display program usage';
		$this->appendOption($G_5_help);
	}
}
...
namespace 
{
	...
	class Application
	{
		...
		{
			$info = new Program\buildPharProgramInfo();
			...
			foreach ($extensions as $extension)
			{
				...
				{
					error_log($ex . ' extension not loaded' . PHP_EOL);
					return (1);
				}
			}

			$xml = new \DOMDocument('1.0', 'utf-8');
			$xml->load($result->xmlProgramDescriptionPath());

			$version = $xml->documentElement->getAttributeNode('version')->value;
			...
			) as $attribute)
			{
				if ($xml->documentElement->hasAttribute($attribute))
					$metadata[$attribute] = $xml->documentElement->getAttribute($attribute)->value;
			}

			if (!$result->skipValidation->isSet)
			{
				$xsdFile = $result->nsxmlPath() . '/xsd/program/' . $version . '/program.xsd';
				if (!file_exists($xsdFile))
				{
					error_log('XSD schema not found.');
					return 1;
				}
			...
			if (self::buildPhar($result, $metadata, $xml))
			{
				if ($result->chmod->isSet)
				{
					$v = intval($result->chmod());
					$u = intval($v / 100);
					$g = intval($v / 10) % 10;
					$o = $v % 10;
					$v = $o + ($g * 8) + ($u * (64));
					chmod($result->outputScriptFilePath(), $v);
				}
			}
		}

		// main
		private static function buildPhar(\Parser\ProgramResult $result, $metadata, $xml)
		{
			$xsltOptions = array ();
			if ($result->parserNamespace->isSet)
				$xsltOptions['prg.php.parser.namespace'] = $result->parserNamespace();

			if ($result->programNamespace->isSet)
				$xsltOptions['prg.php.programinfo.namespace'] = $result->programNamespace();

			$applicationAlias = pathinfo($result->outputScriptFilePath, \PATHINFO_FILENAME);

			$applicationContent = file_get_contents($result->programFilePath);

			$phar = new \Phar($result->outputScriptFilePath, \FilesystemIterator::CURRENT_AS_FILEINFO | \FilesystemIterator::KEY_AS_FILENAME, $applicationAlias);

			$files = array (
					'parser' => array (
							'xsl' => $result->nsxmlPath() . '/xsl/program/' . $metadata['version'] . '/php/parser.xsl',
							'localname' => '__parser.php'
					),
					'info' => array (
							'xsl' => $result->nsxmlPath() . '/xsl/program/' . $metadata['version'] . '/php/programinfo.xsl',
			...
			foreach ($files as $key => $value)
			{
				...
				$phar->addFromString($value['localname'], $xslt->transformToXml($xml));
				$applicationLoader .= 'require("phar://' . $applicationAlias . '/' . $value['localname'] . '");' . PHP_EOL;
			}

			$applicationLoader .= '} // namespace' . PHP_EOL;
			$applicationLoader .= '?>' . PHP_EOL;

			$phar->addFromString('index.php', $applicationLoader . $applicationContent);
